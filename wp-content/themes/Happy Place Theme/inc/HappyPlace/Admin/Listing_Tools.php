...
use HappyPlace\Utilities\Geocoding;
...
class Listing_Tools {
    ...
    public function render_tools_page(): void {
        ...
        <div class="wrap">
            ...
            <div class="card">
                <h2><?php esc_html_e('Geocoding Tools', 'happy-place'); ?></h2>
                <p><?php esc_html_e('Geocode all listings that don\'t have coordinates.', 'happy-place'); ?></p>
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <button type="submit" class="button button-primary">
                        <?php esc_html_e('Geocode All Listings', 'happy-place'); ?>
                    </button>
                </form>
                
                <?php if (isset($_GET['geocoded'])) : ?>
                    <div class="notice notice-success">
                        <p><?php printf(__('Geocoded %d listings.', 'happy-place'), intval($_GET['geocoded'])); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    public function handle_geocode_all(): void {
        ...
        foreach ($listings as $listing) {
            if (Geocoding::instance()->force_geocode_listing($listing->ID)) {
                $geocoded++;
            }
        }
